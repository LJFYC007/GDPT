...
class GeneralizedPoissonReconstructor:
    ...
    def reconstructChannel(self, img, variance, gradX, gradY, varX, varY, channel=None):
        ...
        if self.verbose:
            x, info = cupyx.scipy.sparse.linalg.cg(A, b, x0=cp.array(img.ravel()), M=precond, tol=1e-10, maxiter=10, callback=callback)
        else:
            x, info = cupyx.scipy.sparse.linalg.cg(A, b, x0=cp.array(img.ravel()), M=precond, tol=1e-10, maxiter=10)

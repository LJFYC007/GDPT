...
def render_graph_PathTracer():
    ...
    AccumulatePassDGradientX = createPass("AccumulatePass", {'enabled': True, 'precisionMode': 'Single'})
    g.addPass(AccumulatePassDGradientX, "AccumulatePassDGradientX")
    AccumulatePassDGradientY = createPass("AccumulatePass", {'enabled': True, 'precisionMode': 'Single'})
    g.addPass(AccumulatePassDGradientY, "AccumulatePassDGradientY")
    g.addEdge("PathTracer.gradientX", "AccumulatePassX.input")
    g.addEdge("PathTracer.gradientY", "AccumulatePassY.input")
    g.addEdge("PathTracer.dGradientX", "AccumulatePassDGradientX.input")
    g.addEdge("PathTracer.dGradientY", "AccumulatePassDGradientY.input")
    g.markOutput("AccumulatePassX.output")
    g.markOutput("AccumulatePassY.output")
    g.markOutput("AccumulatePassDGradientX.output")
    g.markOutput("AccumulatePassDGradientY.output")
    ...
    PostProcessDGradientX = createPass("PostProcess", {'sigma': 3.0, 'kernelWidth': 11})
    g.addPass(PostProcessDGradientX, "PostProcessDGradientX")
    g.addEdge("AccumulatePassDGradientX.variance", "PostProcessDGradientX.Input")
    g.markOutput("PostProcessDGradientX.Output")

    PostProcessDGradientY = createPass("PostProcess", {'sigma': 3.0, 'kernelWidth': 11})
    g.addPass(PostProcessDGradientY, "PostProcessDGradientY")
    g.addEdge("AccumulatePassDGradientY.variance", "PostProcessDGradientY.Input")
    g.markOutput("PostProcessDGradientY.Output")
